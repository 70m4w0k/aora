...
export async function completeQuest(form) {
  try {
    ...
    // Update user progress (with penalty)
    await updateUserProgress(form.userId, form.householdId, form.xpEarned || 10, form.arcId, 'quest', penaltyXP);
...
// Update User Progress
export async function updateUserProgress(userId, householdId, xpEarned, arcId = null, completionType = null, penaltyXP = 0) {
  try {
    const progress = await getUserProgress(userId, householdId);
